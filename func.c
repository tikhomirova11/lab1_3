...
persone* where(persone* list, int n, bool* (*m)(persone*))
{
    struct persone * newPerson;
    newPerson = (struct persone*)malloc(n * sizeof(struct persone));

    for (int i = 0; i < n; i++)
    {
        if (m(list + i) == true)
        {
            (newPerson + i)->id.seria = (list + i)->id.seria;
            (newPerson + i)->id.nomer = (list + i)->id.nomer;
            for (int j = 0; j < 20; j++)
            {
                (newPerson + i)->firstname[j] = (list + i)->firstname[j];
                (newPerson + i)->lastname[j] = (list + i)->lastname[j];
            }
            (newPerson + i)->bd.date = (list + i)->bd.date;
            (newPerson + i)->bd.month = (list + i)->bd.month;
            (newPerson + i)->bd.year = (list + i)->bd.year;

            printf("\n %s %s", (newPerson + i)->firstname, (newPerson + i)->lastname);
            printf(" дата рождения: %d %d %d, Person ID: %d %d ", (newPerson + i)->bd.date, (newPerson + i)->bd.month, (newPerson + i)->bd.year, (newPerson + i)->id.seria, (newPerson + i)->id.nomer);
...
persone* reduce(persone* list, int n, int* (*R)(persone*), int constant)
{
    for (int i = 0; i < n; i++)
    {
        int g = R((list + i)->bd.date, (list + i)->bd.year);
        constant = constant + g;
    }
    printf("\n %d", constant);
    return constant;
...
persone* FuncAprilAll(persone* list, int n)
{
    struct persone* newPerson = NULL;
    int count = 0;
    int constant = 10;

    for (int i = 0; i < n; i++)
    {
        if (FuncAprilForWhere(list) == true)  count++;
    }

    newPerson = where(list, n, FuncAprilForWhere);

    printf("\n Константа %d", constant);


    reduce(map(newPerson, count, FuncAprilForMap), count, FuncAprilForReduce, 10);
}